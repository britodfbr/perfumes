...
PagSeguro.prototype.atualizaCarrinho = function() {
  ...
  for (i in this.carrinho) {
    ...
    tabela += '<tr><td>'+this.carrinho[i].quant+'</td>'
    tabela += '<td>'+this.carrinho[i].descr+'</td>'
    tabela += '<td>'+this.moeda(this.carrinho[i].valor)+'</td>'
    tabela += '<td>'+this.moeda(this.carrinho[i].quant*this.carrinho[i].valor)+'</td>
    tabela += '<td> <a href="#" onclick="pgs.remover(\''+this.carrinho[i].id+'\');pgs.atualizaCarrinho(); return false"> Remover </a></td></tr>'
