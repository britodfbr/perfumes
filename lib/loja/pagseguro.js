...
PagSeguro.prototype.atualizaCarrinho = function() {
  ...
  for (i in this.carrinho) {
    ...
    tabela += '<tr><td>'+this.carrinho[i].quant+'</td><td>'
    tabela += this.carrinho[i].descr+'</td><td>'
    tabela += this.moeda(this.carrinho[i].valor)+'</td><td>'
    tabela += this.moeda(this.carrinho[i].quant*this.carrinho[i].valor)
    tabela += '</td><td><a href="#" onclick="pgs.remover(\''+this.carrinho[i].id+'\');pgs.atualizaCarrinho(); return false"> Remover </a></td></tr>'
