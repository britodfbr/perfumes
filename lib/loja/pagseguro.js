...
PagSeguro.prototype.atualizaCarrinho = function() {
  ...
  for (i in this.carrinho) {
    ...
    tabela += '<tr><td>'+this.carrinho[i].quant+'</td><td>'+this.carrinho[i].descr+'</td><td>'+this.moeda(this.carrinho[i].valor)+'</td><td>'+this.moeda(this.carrinho[i].quant*this.carrinho[i].valor)+'</td><td><a href="#" onclick="pgs.remover(\''+this.carrinho[i].id+'\');pgs.atualizaCarrinho(); return false"> < img src="'+this.configuracoes.rmBotao+'" alt="Remover item" /> </a></td></tr>'
