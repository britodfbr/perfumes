...
PagSeguro.prototype.atualizaCarrinho = function() {
  var x=total=0
  var tabela = '<table><thead><th>Quantia</th><th>Descrição</th><th>Vlr Unitário</th><th>Vlr Final</th></tr></thead><tbody>'
  ...
  for (i in this.carrinho) {
    ...
    tabela += '<tr><td>'+this.carrinho[i].quant+'</td><td>'+this.carrinho[i].descr+'</td><td>'+this.moeda(this.carrinho[i].valor)+'</td><td>'+this.moeda(this.carrinho[i].quant*this.carrinho[i].valor)+'</td><td><a href="#" onclick="pgs.remover(\''+this.carrinho[i].id+'\');pgs.atualizaCarrinho(); return false"> <img alt='Remover item' class='icon-action' height='15' src='https:\/\/incolume.googlecode.com/svn/trunk/imagens/icon18_email.gif' width='41'/> </a></td></tr>'
