...
PagSeguro.prototype.atualizaCarrinho = function() {
  ...
  for (i in this.carrinho) {
    ...
    tabela += '<tr><td>'+this.carrinho[i].quant+'</td><td>'+this.carrinho[i].descr+'</td><td>'+this.moeda(this.carrinho[i].valor)+'</td><td>'+this.moeda(this.carrinho[i].quant*this.carrinho[i].valor)+'</td><td><a href="#" onclick="pgs.remover(\''+this.carrinho[i].id+'\');pgs.atualizaCarrinho(); return false"> <img alt="Remover item" class="icon-action" height="15" src="'+this.configuracoes.img_base+'icon18_email.gif width="41"/> </a></td></tr>'
