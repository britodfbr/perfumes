PagSeguro = function (configuracoes, loja) {
  ...
  this.vitrine(this.configuracoes.inicio, document.getElementById(this.configuracoes.id_vitrine))
  setTimeout(this.atualizaCarrinho, 1)
}

PagSeguro.prototype.vitrinemenu = function(elm) {
  ret='<ul>'
  for (i in this.loja)
    ret+='<li><a href="#" onclick="pgs.vitrine(\''+i+'\', document.getElementById(pgs.configuracoes.id_vitrine)); return false">'+i+'</a></li>'
  ret+='</ul>'
  if (elm==undefined) document.write(ret)
  else elm.innerHTML = ret
}

PagSeguro.prototype.vitrine = function(grupo, elm) {
  var item
  var ret
