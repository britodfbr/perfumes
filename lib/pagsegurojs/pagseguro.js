...
PagSeguro.prototype.main = function(grupo, elm) {
  ...
  for (var i=0;i<this.loja[grupo].length;i++) {
    ...
    if (this.configuracoes.imagem)
      ret += '<dd class="imagem"><img src="imagem/'+item.img+'.'+this.configuracoes.formatoimg+'" /></dd>'
    ret += '<dd class="referencia"><span>Referência:</span> '+item.ref+'</dd>'
    ret += '<dd class="referencia"><span>Genêro:</span> '+item.genero+'</dd>'
